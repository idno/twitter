...
class Brevity
{
    ...
    function __construct() {
      ...
      $host = '(?:[a-z0-9][a-z0-9\-]*\.)+(?:' . implode('|', $this->tlds) . ')(?::\d{2,6})?';
      $path = '\/[\w\/.\-_~.;:%?!@$#&()=+]*';
    ...
    private function weightedCharLengthAt($str, $index)
    {
      ...
      $weight = $this->weights['defaultWeight'];
      foreach ($this->weights['ranges'] as $range) {
          if ($point >= $range['start'] && $point <= $range['end']) {
            $weight = $range['weight'];
            break;
          }
      }
      return floor($weight / $this->weights['scale']);
    }
